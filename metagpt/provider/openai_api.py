...
class OpenAIGPTAPI(BaseGPTAPI, RateLimiter):
    ...
    def _cons_kwargs(self, messages: list[dict]) -> dict:
        kwargs = {
            ...
            "stop": None,
            "temperature": 0.7,
            "timeout": int(CONFIG._get("TIMEOUT", 60)),
        }
    ...
    async def _achat_completion_with_tools(
        self, messages: list[dict], tools: list[dict], tool_name: str
    ) -> dict:
        """Call the API with tools and force the model to call a specific tool."""
        kwargs = self._cons_kwargs(messages)
        kwargs["tools"] = tools
        kwargs["tool_choice"] = {"type": "function", "function": {"name": tool_name}}
        # streaming is not supported with tool_choice forced, disable it
    ...
    async def aask_with_tools(
        ...
        """Ask the LLM and force it to respond via a tool call.

        Returns the parsed JSON dict from the tool call arguments.
        """
        import json as _json

        if system_msgs:
            messages = [self._system_msg(s) for s in system_msgs] + [
                self._user_msg(msg)
            ]
        else:
            messages = [self._default_system_msg(), self._user_msg(msg)]

        rsp = await self._achat_completion_with_tools(messages, tools, tool_name)
        tool_calls = rsp["choices"][0]["message"].get("tool_calls", [])
        if not tool_calls:
            raise ValueError(f"Model did not return a tool call for tool '{tool_name}'")
