...
class RoleContext(BaseModel):
    ...
    env: "Environment" = Field(default=None)  # default: 设置静态的、固定的默认值
    memory: Memory = Field(
        default_factory=Memory
    )  # default_factory​：接收一个可调用对象 ，每次创建实例时都会调用这个函数生成新的默认值
...
class Role:
    ...
    async def _observe(self) -> int:
        ...
        for i in env_msgs:
            if (
                i.restricted_to != ""
                and self.profile not in i.restricted_to
                and self.name not in i.restricted_to
            ):
