...
from werewolf_game.actions import (
    ACTIONS,
    InstructSpeak,
    Speak,
    Reflect,
...
from werewolf_game.actions.experience_operation import (
    AddNewExperiences,
    RetrieveExperiences,
)
...
class BasePlayer(Role):
    ...
    async def _act(self):
        ...
        hard_facts, soft_signals, reflection = (
            await Reflect().run(
        ...
        if isinstance(todo, Speak):
            ...
            restricted_to = (
                f"Moderator,{self.profile}"  # 给Moderator发送使用特殊技能的加密消息
            )
    ...
    def get_all_memories(self) -> str:
        ...
        memories = [
            f"{m.sent_from}: {re.sub(time_stamp_pattern, '', m.content)}"
            for m in memories
        ]  # regex去掉时间戳
        memories = "\n".join(memories)
        return memories
    ...
    def get_latest_instruction(self) -> str:
        return self._rc.important_memory[
            -1
        ].content  # 角色监听着Moderator的InstructSpeak，是其重要记忆，直接获取即可

    def get_game_setup(self) -> str:
        """从记忆中提取游戏初始配置（第一条 UserRequirement 消息）"""
        memories = self._rc.memory.get()
        for m in memories:
            if "Game setup:" in m.content:
                return m.content
        return ""
    ...
    def record_experiences(self, round_id: str, outcome: str, game_setup: str):
        experiences = [
            exp for exp in self.experiences if len(exp.reflection) > 2
        ]  # not "" or not '""'
